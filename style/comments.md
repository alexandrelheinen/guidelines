...
say so in the project's own guideline file: do not mix silently.
...
## Public API documentation is mandatory at library/module boundaries
...
mandatory, not optional, because the reader cannot fall back on reading the
implementation.
...
- A test's `describe`/`it` or function name, if it already states the
  behavior being verified: do not add a comment above it repeating the name.
- A type hint or the parameter list itself: do not restate types in prose.
...
above it explaining why it is necessary. This is the one case where a

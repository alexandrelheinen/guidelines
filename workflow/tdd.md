...
universal constant — but don't merge a project with no coverage gate at
...
for a specific case, that's a decision worth a line in `docs/decisions.md`,
...
don't let a codebase accumulate permanently-skipped tests.

## No silent skips

A skipped or expected-to-fail test must always carry a reason that names
what's blocking it (a requirement id, an issue link, or a one-line
...
JS/TS. Pick one convention per project and apply it uniformly — don't mix
